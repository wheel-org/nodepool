...
var TABLE_HOLE_RADIUS_CENTER = PHYSICAL_SCALE * 5.3;
var TABLE_HOLE_RADIUS_CORNER = PHYSICAL_SCALE * 6.5;
...
function drawTableAndBalls() {
    ...
    context.fillRect(
        table_x + TABLE_WOOD_WIDTH,
        table_y + TABLE_WOOD_WIDTH,
        TABLE_WIDTH - 2 * TABLE_WOOD_WIDTH,
        TABLE_HEIGHT - 2 * TABLE_WOOD_WIDTH
    );
    // Table Green No Cushion
    context.fillStyle = createGradient(0, 0, screenWidth, screenHeight, '#288200', '#216d00');
    context.fillRect(
        table_x + table_wood_and_cushion,
        table_y + table_wood_and_cushion,
        TABLE_WIDTH - 2 * table_wood_and_cushion,
        TABLE_HEIGHT - 2 * table_wood_and_cushion
    );
    TABLE_CUSHION_WIDTH;
    // Draw Holes, Centered on Wood + Cuahion
    context.fillStyle = '#000';
    drawCircle(table_x + TABLE_HOLE_OFFSET, table_y + TABLE_HOLE_OFFSET, TABLE_HOLE_RADIUS_CORNER);
    drawCircle(
        table_x + TABLE_WIDTH - TABLE_HOLE_OFFSET,
        table_y + TABLE_HEIGHT - TABLE_HOLE_OFFSET,
        TABLE_HOLE_RADIUS_CORNER
    );
    drawCircle(table_x + TABLE_HOLE_OFFSET, table_y + TABLE_HEIGHT - TABLE_HOLE_OFFSET, TABLE_HOLE_RADIUS_CORNER);
    drawCircle(table_x + TABLE_WIDTH - TABLE_HOLE_OFFSET, table_y + TABLE_HOLE_OFFSET, TABLE_HOLE_RADIUS_CORNER);
    drawCircle(table_x + TABLE_WIDTH / 2, table_y + TABLE_HEIGHT - TABLE_HOLE_OFFSET, TABLE_HOLE_RADIUS_CENTER);
    drawCircle(table_x + TABLE_WIDTH / 2, table_y + TABLE_HOLE_OFFSET, TABLE_HOLE_RADIUS_CENTER);
    // White Line
    context.beginPath();
